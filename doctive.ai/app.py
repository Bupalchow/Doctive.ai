...
def predict():
   ...
   d  = [ ]
   put_html(f"<h1>WELCOME TO DOCTIVE.AI</h1>")
   for i in range(1,11):
      Symptom1 = select('Symptom '+str(i),symptoms,name = f's{i}')
      d.append(Symptom1)
   d = input_group('Enter your symptoms',d)
   d= list(d.values())
   ...
   put_html(f"<h1>You have {prediction[0]}</h1>")
   put_html(f"<h6 style='color:green'>This Just a Test Model All The Results Are Not 100% Accurate (App Still Underdevelopment)</h6>")
   put_html(f"<h5 style='color:red;'>Might produce random values if no symptoms are given</h5>")
   
   
   put_column([put_html(f'<h2 >Description of Disease:</h2> '),
               put_html(list(a[a['Disease'] == prediction[0]]['Description'])[0]),
               ])
   
   
   
   put_html('<a href="/" style="background-color:black;margin-left:350px;color:white;;padding :8px;border-radius:5px;font-size:30px;text-decoration:none;box-shadow:0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19)">Back</a>')
